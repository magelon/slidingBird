...
public class waterShapes : MonoBehaviour
{
    ...
    void Start()
    {
        poolManager.instance.CreatePool(land, 1000);
        startTime = 0;
        minimum = 1f;
        maximum = inpactScale;
        ...
        */
        ...
        for (int i = 2; i < vertices.Length - 1; i++)
        {
            ...
            if (i == 2 || i % 2 != 0)
            {
                ...
                vertex.y += Mathf.PerlinNoise(baseHeight[i].x + transform.position.x + noiseWalk, (baseHeight[i].y + transform.position.y) + scale); //+ Mathf.Sin(Time.time * speed)) * scale;
                vertices[i] = vertex;

                poolManager.instance.ReuseObject(land, new Vector3(vertex.x + transform.position.x, vertex.y + transform.position.y, 0), Quaternion.identity);
